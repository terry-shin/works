...
from usrlib.rdb.model import ProductData  # テストコード実行時エラー発生のため無効化
...
def publish_keepa_call_product(asin_list):
    ...
    for target_asin in asin_list:
        ...
        check_product_data = ProductData.get_product_data(target_asin)

        if not check_product_data or check_product_data.updated_at < check_datetime:
            param_list = {
                "api": "product",
                "asin": [target_asin]
            }
            ...
            print(f"product data exist and updated: {target_asin}")
    return
